...
const ResultNBP = props => {
    function show(){
        ...
        if(list.style.display!=="block"){
            ...
            button.innerText="Pokaż wszystkie"
        }
    }
    ...
    for(var i=ratesLength-2;i>=0; i--){
        index++;
        elements.push(<div className="Mid" key={index}>Data: {rates[i].effectiveDate} średni kurs: {rates[i].mid}</div>);
    }
    if (!err && currency) {
        content = (
            <div>Kursy dla: {currency}
                ...
                <div></div>
                <div className="Mid" key={index}>Data: {rates[ratesLength-1].effectiveDate} średni kurs: {rates[ratesLength-1].mid}</div>
                <div id="lista" className="List">{elements}</div>
                <Button id="showRates" onClick={show}>Pokaż wszystkie</Button>
            </div>
        )
    }
    return (
